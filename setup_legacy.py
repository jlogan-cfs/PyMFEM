...
repos_sha = {
    "mfem": "c8ef9676e83c93a9037dbdf3bf0f2d741321e632",    # v4.9.1
...
if platform == "linux" or platform == "linux2":
    dylibext = '.so'
    # Wheel build works on linux using chrpath command
    try:
    ...
    # OS X
    dylibext = '.dylib'
    # Wheel build works on macOS using install_name_tool
    try:
        from wheel.bdist_wheel import bdist_wheel as _bdist_wheel
        haveWheel = True
    except ImportError:
...
def cmake_make_mfem(serial=True):
    ...
    if do_bdist_wheel:
        ...
        for x in os.listdir(ex_dir):
            ...
            if platform == "darwin":
                # On macOS, use install_name_tool to set rpath
                command = ['install_name_tool', '-add_rpath', '@loader_path/../lib', path]
            else:
                # On Linux, use chrpath to set rpath
